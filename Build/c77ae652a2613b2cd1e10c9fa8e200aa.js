...
var ASM_CONSTS = {
  3908064: function() {return Module.webglContextAttributes.premultipliedAlpha;},  
 3908125: function() {return Module.webglContextAttributes.preserveDrawingBuffer;},  
 3908189: function() {return Module.webglContextAttributes.powerPreference;},  
 3908247: function() {Module['emscripten_get_now_backup'] = performance.now;},  
 3908302: function($0) {performance.now = function() { return $0; };},  
 3908350: function($0) {performance.now = function() { return $0; };},  
 3908398: function() {performance.now = Module['emscripten_get_now_backup'];}
};
  ...
  async function _CheckPremium(url, token) {
      if (
        window &&
        window.Telegram &&
  ...
  function _CloseMiniApp(message) {
      if (window.Telegram.WebApp && window.unityInstance) {
        const val = UTF8ToString(message);
        window.Telegram.WebApp.close();
      } else {
        console.log({ CloseMiniApp: "CloseMiniApp not defined" });
      }
    }
  ...
  function _ConsumableUsed(consumable) {
      if (
        ...
        const data = { consumableType: consumableType };
        window.socket.emit("useConsumable", data);
        // console.log({ data });
      } else {
  ...
  async function _GenerateTONInvoice(baseUrl, token, inv_t) {
      if (
        window &&
        window.Telegram &&
        window.Telegram.WebApp &&
        ...
        try {
          ...
          if (window.tonConnectUI && window.tonConnectUI.connected) {
            ...
            address = window.tonConnectUI.account.address;
          }
          if (window.OKX_TON && window.OKX_TON.connected) {
            walletType = "okx-wallet";
            address = window.OKX_TON._wallet.account.address;
          }
          ...
          console.log({ GenerateTONInvoice: data });
        ...
        } catch (GenerateTONInvoiceErr) {
          console.log({ GenerateTONInvoiceErr });
        }
      } else {
        console.log({ GenerateTONInvoice: "Error" });
      }
    }
  ...
  function _GetAllKeys() {
      if (window.Telegram.WebApp && window.unityInstance) {
        try {
          window.Telegram.WebApp.CloudStorage.getKeys(function (err, values) {
            console.log("GetAllKeysFC", err, values);
            if (err) {
              console.log({ GetAllKeysErr: err });
            } else {
              console.log({ GetAllKeysValues: values });
            }
          });
        } catch (e) {
          console.log({ e });
  ...
  function _GetKey(key) {
      if (window.Telegram.WebApp && window.unityInstance) {
        const k = UTF8ToString(key);
        try {
          window.Telegram.WebApp.CloudStorage.getItem(k, function (err, saved) {
            if (err) {
              console.log({ err });
            } else {
              console.log({ saved });
      ...
      if (window && window.Telegram && window.Telegram.WebApp) {
        try {
          window.Telegram.WebApp.CloudStorage.getItem(
            ...
            function (err, value) {
              if (err) {
                console.log({ getPointsErrorCloudStorage: err });
              } else {
                console.log({ getPointsValue: value });
                window.unityInstance.SendMessage(
                  "RequestHandler",
                  "SetDailyPoints",
                  value
  ...
  function _GetValues(keys) {
      if (window.Telegram.WebApp && window.unityInstance) {
        try {
          ...
          window.Telegram.WebApp.CloudStorage.getKeys(function (err, vals) {
            if (err) {
              console.log({ err });
            } else {
              if (vals.length > 0) {
  ...
  function _HapticFeedback(level) {
      if (window && window.Telegram && window.Telegram.WebApp) {
        try {
          const val = UTF8ToString(level);
          window.Telegram.WebApp.HapticFeedback.notificationOccurred(val);
        } catch (HapticFeedbackError) {
          console.log({ HapticFeedbackError });
        }
      }
    }

  function _InitSocket(url, token) {
      if (window.Telegram.WebApp && window.unityInstance) {
        const ws = UTF8ToString(url);
        const tkn = UTF8ToString(token);
        if (io) {
          let socket = io(ws, {
          ...
          window.socket.on("connect", () => {
            ...
            //   );
            // });
            // window.socket.on("consumableUsed", (response) => {
            //   console.log("consumableUsed", response);
            // });
          ...
          const response = await fetch(
            "https://api.joincommunity.xyz/external/events",
            {
              ...
              headers: {
                "Content-Type": "application/json",
                "api-key":
                  "kIkJVe32GUbPR0gpSBVaqBiYyxP5c7HgDLS2UYOvSKjR03Am3UBx39yfJWmuZsLaUAseo1Xccny85tHSL2kaWMz0PY5KWVd7xrwgq8Td4mBGYOPmFP8YDpC2cQnoOIRLGy4CSZxuQEbyY1VlmnlA8nTtK",
              },
  ...
  async function _OKXOpenWallet(returnUrl) {
      try {
        if (
          window &&
          window.Telegram &&
      ...
      try {
        if (
          ...
          const okxStatus = () => {
            if (!window.OKX_TON || !window.OKX_TON.connected) {
              try {
                window.OKX_TON.onStatusChange(
                  (walletInfo) => {
                    if (walletInfo) {
                      console.log("OKX Connected", walletInfo);
                      window.Telegram.WebApp.CloudStorage.getItem(
                        "okxConnect",
                        function (err, saved) {
                          if (err) {
          ...
          const connectOKX = async () => {
            if (!window.OKX_TON || !window.OKX_TON.connected) {
              const OKX_TON = new OKXTonConnectSDK.OKXTonConnect({
                metaData: {
                  name: "Plank Pushers",
                  icon: "https://i.ibb.co/5RmjhxS/180.png",
                },
              });
              window.OKX_TON = OKX_TON;
              let connectData = {
                redirect: null,
                openUniversalLink: true,
              };
              let url = await window.OKX_TON.connect(connectData);
              console.log(url);
              console.log(" OKX Wallet Connected ", url);
              ...
              window.OKX_TON.restoreConnection();
              window.fitScreen();
              if (window.OKX_TON.connected) {
                window.Telegram.WebApp.showPopup(
                  {
                    title: "Disconnect your OKX wallet",
                    message: "Your wallet will be disconnected.",
                    buttons: [
                      { id: "1", type: "close", text: "Close" },
                      { id: "2", type: "destructive", text: "Confirm" },
          ...
          if (window.isPC()) {
            window.Telegram.WebApp.showPopup(
              {
                title: "PC Not Supported Yet!",
                message: "Please use a mobile device to connect OKX wallet.",
              },
              function () {
                window.unityInstance.SendMessage(
                  "GameManager",
  ...
  function _OpenExternalLink(url) {
      if (window.Telegram.WebApp && window.unityInstance) {
        const val = UTF8ToString(url);
        window.Telegram.WebApp.openLink(val);
      }
    }
  ...
  async function _QuestClaim(url, token, key) {
      if (
        ...
        try {
          ...
          const data = await response.json();
          window.Telegram.WebApp.CloudStorage.setItem(k, "2");
        } catch (questClaim) {
          console.log({ questClaim });
        }
      }
    }
  ...
  async function _SendGetInvoiceData(url, token) {
      if (
        window &&
        window.Telegram &&
        window.Telegram.WebApp &&
        ...
        try {
          ...
          if (window.tonConnectUI.connected) {
            ...
            values.address = window.tonConnectUI.account.address;
            values.walletType = "ton-connect";
          } else if (window.OKX_TON && window.OKX_TON.connected) {
            values.connected = true;
            values.address = window.OKX_TON._wallet.account.address;
            values.walletType = "okx-wallet";
          } else {
            values.connected = false;
            values.address = null;
            values.walletType = null;
          }
          const response = await fetch(l, {
        ...
        } catch (sendGetInvoiceData) {
          console.log({ sendGetInvoiceData });
        }
      } else {
        console.log({ sendGetInvoiceData: "Not connected" });
      }
    }

  function _SendScore(coins, score, points) {
      if (
        window &&
        window.Telegram &&
        window.Telegram.WebApp &&
            ...
            function (err, values) {
              if (err) {
                ...
                const newArr = arr.map((item) => {
                  if (item[0] === "score") {
                    return [item[0], item[1] + Number(score)];
                  } else if (item[0] === "coins") {
                    return [item[0], item[1] + Number(coins)];
                  } else if (item[0] === "points") {
                    return [item[0], item[1] + Number(points)];
                  } else {
                    return item;
                  }
                });
        ...
        } catch (sendScoreBError) {
          console.log({ sendScoreBError });
        }
      }
    }
  ...
  function _SessionToken() {
      if (window.Telegram.WebApp && window.unityInstance) {
        var initData = JSON.stringify(window.Telegram.WebApp.initData);
  ...
  function _ShowConnectWalletPopup() {
      if (
        window &&
        window.unityInstance &&
        window.Telegram &&
        ...
        window.Telegram.WebApp.showPopup(
          {
            ...
            buttons: [{ id: "2", type: "destructive", text: "Close" }],
          },
          function (val) {
            window.unityInstance.SendMessage(
              "GameManager",
  ...
  async function _Validate(urlPtr) {
      if (window.Telegram.WebApp && window.unityInstance) {
        try {
          const url = UTF8ToString(urlPtr); // Converts the pointer to a JS string
          ...
          if (contentType && contentType.includes("application/json")) {
            ...
            if (
              ...
              if (userType === 1 || userType === 2) {
                eruda.init(); // Ensure eruda is loaded
              }
            }
  
            window.Telegram.WebApp.CloudStorage.setItem("userType", userType);
            window.Telegram.WebApp.CloudStorage.setItem("token", data.token);
            ...
            window.Telegram.WebApp.CloudStorage.getItem(
              ...
              function (err, value) {
                if (err) {
                  ...
                  const systemDate = new Date(value);
                  const today = new Date(data.questDate);
                  if (systemDate.toDateString() !== today.toDateString()) {
                    window.Telegram.WebApp.CloudStorage.setItem(
                      "questDate",
                      data.questDate,
                      function (error, result) {
  ...
  async function _VerifyTONTransaction(
      baseUrl,
      token,
      payload,
      boc,
      ...
      if (
        ...
        try {
          ...
          const tkn = UTF8ToString(token);
          const ply = UTF8ToString(payload);
          const bc = UTF8ToString(boc);
          const to = UTF8ToString(time_out);
          const wallet = UTF8ToString(walletType);
          ...
          console.log({ data });
          if (data.done) {
            window.Telegram.WebApp.showAlert("Transaction Saved", () =>
              window.unityInstance.SendMessage(
                "RequestHandler",
...
var asmLibraryArg = {
  ...
  "CloseMiniApp": _CloseMiniApp,
  "CloseWebFrame": _CloseWebFrame,
  "ConsumableUsed": _ConsumableUsed,
  "DeleteKey": _DeleteKey,
  "DisconnectSocket": _DisconnectSocket,
  "EnableEruda": _EnableEruda,
  "FitToScreen": _FitToScreen,
  "GenerateTONInvoice": _GenerateTONInvoice,
  ...
  "JoinGame": _JoinGame,
  "NewOpenWallet": _NewOpenWallet,
  "NotCoinTask": _NotCoinTask,
  "OKXOpenWallet": _OKXOpenWallet,
  "OKXWalletConnected": _OKXWalletConnected,
  "OkxWalletInitalize": _OkxWalletInitalize,
  "OpenExternalLink": _OpenExternalLink,
  "OpenStarsInvoice": _OpenStarsInvoice,
  "OpenTGLink": _OpenTGLink,
  "OpenTelegramLink": _OpenTelegramLink,
  "OpenWebFrame": _OpenWebFrame,
  "QuestClaim": _QuestClaim,
  "SaveWalletData": _SaveWalletData,
  "Screenshot": _Screenshot,
  "SendDataToBackend": _SendDataToBackend,
  "SendGetInvoiceData": _SendGetInvoiceData,
  "SendScore": _SendScore,
  "SendTransaction": _SendTransaction,
  "SessionToken": _SessionToken,
  "SetKey": _SetKey,
  "ShareInviteLink": _ShareInviteLink,
  "ShowConnectWalletPopup": _ShowConnectWalletPopup,
  "Validate": _Validate,
  "VerifyTONTransaction": _VerifyTONTransaction,
